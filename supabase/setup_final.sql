-- ============================================================
-- SCRIPT FINAL DE CONFIGURAÇÃO DO SUPABASE
-- Resolve erro "function is not unique" droppando todas as versões
-- ============================================================

-- 1. PRIMEIRO: Dropar TODAS as versões das funções existentes
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Dropar todas as versões de get_course_availability
    FOR func_record IN 
        SELECT oid::regprocedure as func_name 
        FROM pg_proc 
        WHERE proname = 'get_course_availability' 
        AND pronamespace = 'public'::regnamespace
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name;
    END LOOP;
    
    -- Dropar todas as versões de get_all_courses_admin
    FOR func_record IN 
        SELECT oid::regprocedure as func_name 
        FROM pg_proc 
        WHERE proname = 'get_all_courses_admin' 
        AND pronamespace = 'public'::regnamespace
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name;
    END LOOP;
    
    -- Dropar todas as versões de register_participant_with_courses
    FOR func_record IN 
        SELECT oid::regprocedure as func_name 
        FROM pg_proc 
        WHERE proname = 'register_participant_with_courses' 
        AND pronamespace = 'public'::regnamespace
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name;
    END LOOP;
    
    -- Dropar todas as versões de update_course_status
    FOR func_record IN 
        SELECT oid::regprocedure as func_name 
        FROM pg_proc 
        WHERE proname = 'update_course_status' 
        AND pronamespace = 'public'::regnamespace
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name;
    END LOOP;
    
    -- Dropar todas as versões de update_course_capacity
    FOR func_record IN 
        SELECT oid::regprocedure as func_name 
        FROM pg_proc 
        WHERE proname = 'update_course_capacity' 
        AND pronamespace = 'public'::regnamespace
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name;
    END LOOP;
    
    -- Dropar todas as versões de update_all_courses_status
    FOR func_record IN 
        SELECT oid::regprocedure as func_name 
        FROM pg_proc 
        WHERE proname = 'update_all_courses_status' 
        AND pronamespace = 'public'::regnamespace
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name;
    END LOOP;
...
-- 3. Criar tabela de cursos (apenas se não existir - dados são preservados)
...
-- 5. Criar tabela de inscrições (apenas se não existir - dados são preservados)
...
-- 6. Criar tabela de relacionamento (apenas se não existir - dados são preservados)
...
-- 7. Inserir/atualizar cursos (UPSERT - não afeta inscrições)
...
SELECT 'Total de cursos:' AS info, COUNT(*) AS quantidade FROM public.courses;
SELECT 'Total de inscrições:' AS info, COUNT(*) AS quantidade FROM public.registrations;
