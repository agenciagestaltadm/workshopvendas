...
-- Usando starts_at como critério (mantém o primeiro criado com base na data de início)
...
SELECT DISTINCT ON (name) 
    ...
    is_active
FROM public.courses
ORDER BY name, starts_at ASC; -- Manter o mais antigo (primeiro horário)

-- 1.4 Verificar quais serão mantidos
SELECT '=== CURSOS QUE SERÃO MANTIDOS ===' AS info;
SELECT id, name, starts_at, capacity, is_active FROM courses_to_keep;
